...
class PreProcess(object):
	...
	def __init__(self, prompt_max_len, add_question, experiment):
	...
	def processor(self,summary_df,prompt_df):
		...
		for pid, plen, paragraphs  in zip(prompt_ids, _len_prompts, prompts):
			...
			_scores = self._score_paragraphs( _para_list, summaries )
			...
			if plen <= self.prompt_max_len:
				...
				para_list = [para_list[idx] for idx,_,_ in _info]
				_proced_prompts.append('|'.join( para_list ) )
			else:
				_plens = 0
				_ids = [ ]
		...
		if self.experiment:
			summary_df = summary_df.groupby("prompt_id").sample(frac=0.5, random_state=2)
