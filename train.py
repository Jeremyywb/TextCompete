...
# 
from keras.layers import Dense, Input,LSTM as CuDNNLSTM, Embedding, Dropout, Activation, GRU as CuDNNGRU, Conv1D
...
spell_model = gensim.models.KeyedVectors.load_word2vec_format('/content/gdrive/MyDrive/Commpettion/CESS/pretrainedW2V/wiki-news-300d-1M/wiki-news-300d-1M.vec')
...
def load_glove(word_dict, lemma_dict):
    EMBEDDING_FILE = '/content/gdrive/MyDrive/Commpettion/CESS/pretrainedW2V/glove.840B.300d/glove.840B.300d.txt'
...
def load_fasttext(word_dict, lemma_dict):
    EMBEDDING_FILE = '/content/gdrive/MyDrive/Commpettion/CESS/pretrainedW2V/wiki-news-300d-1M/wiki-news-300d-1M.vec'
    def get_coefs(word,*arr): return word, np.asarray(arr, dtype='float32')
...
def load_para(word_dict, lemma_dict):
    EMBEDDING_FILE = '/content/gdrive/MyDrive/Commpettion/CESS/pretrainedW2V/paragram_300_sl999/paragram_300_sl999.txt'
    def get_coefs(word,*arr): return word, np.asarray(arr, dtype='float32')
...
def build_model(embedding_matrix, nb_words, embedding_size=300):
    ...
    predictions = Dense(1, name='output')(conc)
    model = Model(inputs=inp, outputs=predictions)
    adam = optimizers.Adam(learning_rate=learning_rate)
    model.compile(loss='mse', 
        metrics=[
            K.metrics.RootMeanSquaredError(name='rmse')
        ], 
        optimizer=optimizers.AdamW(learning_rate= 6e-3, weight_decay=1e-3))

    return model

start_time = time.time()
print("Loading data ...")
train = pd.read_csv('/content/gdrive/MyDrive/tmpdata/summaries_train.csv')
test = pd.read_csv('/content/gdrive/MyDrive/tmpdata/summaries_test.csv')
